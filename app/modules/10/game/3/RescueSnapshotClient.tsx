...
export default function RescueSnapshotClient({ teamId: teamIdProp = "3" }: { teamId?: string }) {
  ...
  async function submitPhoto() {
    ...
    if(sb){
      try { // @ts-ignore
      await sb.from("photo_submissions").insert({team_id:teamId,module_id:10,game_id:3,storage_path:`team-${teamId}/module10/${step.id}-${Date.now()}.jpg`,caption:caption.trim(),member_label:"Team",media_type:"photo",status:"pending"}); } catch(_){}
    }
    if (stepIdx+1<STEPS.length) { setStepIdx(i=>i+1); }
    else {
      setSaving(true);
      if(sb){ try { // @ts-ignore
      await sb.from("scores").upsert({team_id:teamId,module_id:10,game_id:3,points:next.length*150,time_seconds:0,game_cards:next.length>=STEPS.length?1:0}); } catch(_){} }
      setSaving(false);
      setPhase("results");
    }
  }
  ...
  if (phase==="intro") return (
    <div className="flex flex-col gap-5">
      <div className="flex flex-col items-center gap-3 py-6">
        ...
        <p className="text-[13px] text-gray-500">Module 10 . Game 3 of 4</p>
      </div>
  ...
  if (phase==="results") return (
    <div className="flex flex-col gap-4">
      <div className="rounded-2xl p-6 text-center bg-green-50">
        <p className="text-[14px] font-bold uppercase tracking-wide mb-1 text-green-700">Documentation Complete!</p>
