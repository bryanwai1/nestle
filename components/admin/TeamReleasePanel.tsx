...
export function TeamReleasePanel() {
  ...
  async function load() {
    const { data: completions } = await (supabase as any)
      ...
      .select('team_id, module_id, completed_at')
      .eq('status', 'completed')
    ...
    const teamMap = Object.fromEntries((allTeams ?? []).map((t) => [t.id, t.team_number]));
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-gradient-to-br from-[#0B2545] to-[#0a1d3a] p-5 shadow-xl">
      <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-sky-300/70">Pending Review</p>
      <h3 className="mt-1 text-lg font-bold text-white">Teams Waiting to Continue</h3>
      {paused.length === 0 ? (
        ...
        <>
          <div className="mt-3 space-y-2">
            {paused.map((p) => (
              <div key={`${p.team_id}:${p.module_id}`} className="flex items-center justify-between rounded-xl bg-white/5 px-3 py-2">
                <div>
                ...
                <button onClick={() => release(p.team_id, p.module_id)} disabled={busy !== null}
          ...
          <button onClick={releaseAll} disabled={busy !== null}
